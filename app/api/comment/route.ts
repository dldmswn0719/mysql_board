    ...
    if(req.method === 'POST'){
        ...
        try{
            ...
            if(!parentid || !userid || !username || !content){
                ...
                await db.query<RowDataPacket[]>('insert into boarddata.comment (parentid , userid , username , content) values(?,?,?,?)',[parentid , userid , username , content])
                const [datas] =  await db.query<RowDataPacket[]>('select * from boarddata.comment where parentid = ?',[parentid])
...
export const GET = async (
    ...
    if(req.method === 'GET'){
        try{
            ...
            return NextResponse.json({message : "get요청" , result : results})
        }catch(error){
            return NextResponse.json({error: error})
        }
    }else{
